...
import dattt.account.AccountCreateError;
import dattt.account.AccountDAO;
...
public class SignupServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            }else if(!pass.trim().equals(repass.trim())){
                foundErr = true;
            ...
            boolean result = dao.signUp(user, pass);
            if(result){
                url = LOGIN_PAGE;
            }
        }catch(SQLException ex){
            String msg = ex.getMessage();
            log("SignupServlet   SQL: "+msg);
            if(msg.contains("duplicate")){
                errors.setUsernameIsExisted(user + " is existed");
                request.setAttribute("SIGNUPERRS", errors);
            }
        }catch(NamingException ex){
            log("SignupServlet   Naming: "+ex.getMessage());
        } finally{
